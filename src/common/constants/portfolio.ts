/** 経歴 */
export const CAREER = [
  {
    company: '株式会社リボルブ沖縄',
    entry: '2019/04',
    leave: '2021/03',
    description:
      'Salesforceエンジニアとして2年間プロジェクトに所属しておりました。\n主にSalesforceの標準機能を使用したCRM開発とカスタマイズ開発を行っていました。\nSalesforce開発において、Apex, Visualforce, LWC, SOQL, Lightning Platformを使用しております。',
  },
]

/** 開発サービス */
export const SERVICE = [
  {
    title: 'shikaron',
    devlang: 'next.js',
    link: 'https://www.shikaron.com/',
    description:
      '司法試験の予備試験対策のwebサービスです¥n問題を解いて、論文を記録することができます¥n過去の予備試験合格者が添削をしてくれます',
  },
  {
    title: 'sengyo',
    devlang: 'flutter',
    link: 'https://apps.apple.com/jp/app/sengyo-%E6%8D%8C%E3%81%84%E3%81%9F%E9%AD%9A%E3%82%92%E8%A6%8B%E3%81%9B%E5%90%88%E3%81%86%E3%82%A2%E3%83%97%E3%83%AA/id6450609224',
    description:
      'お魚をさばいて、みんなに自慢するアプリです¥nさばいた魚の写真や料理をした写真をアップすることができます',
  },
  {
    title: 'kensubu',
    devlang: 'flutter',
    link: 'https://apps.apple.com/jp/app/%E5%89%A3%E3%81%99%E3%81%B6/id1567392549',
    description:
      '趣味である、剣道をモチーフにしたアプリです¥nコロナ禍で剣道ができないときに¥n素振りで競ってみるのも面白いと思って¥n作成したアプリです¥n現在130人ほどのユーザが利用してます',
  },
]

/** 経験言語・スキル */
export const SKILLS = [
  {
    name: 'Java',
    experience: '約1年',
    isExperience: true,
    color: 'lightgreen',
    remark: 'API開発',
  },
  {
    name: 'Flutter',
    experience: '約2年',
    isExperience: false,
    color: 'lightblue',
    remark: 'riverpodの使用経験有',
  },
  {
    name: 'firebase',
    experience: '約5年',
    isExperience: false,
    color: 'yellow',
    remark: 'firestore, storage, authentication, functions',
  },
  {
    name: 'Next.js',
    experience: '約2年',
    isExperience: true,
    color: 'red',
    remark: 'recoil, chakra-ui, react-hook-form',
  },
  {
    name: 'Typescript',
    experience: '約2年',
    isExperience: true,
    color: 'blue',
    remark: 'Next.js, React.js',
  },
  {
    name: 'Salesforce',
    experience: '約2年',
    isExperience: true,
    color: 'darkblue',
    remark: 'Apex, Visualforce, LWC, SOQL, Lightning Platform',
  },
]

/** 取得資格 */
export const LICENSE = [
  {
    name: 'Python 3 エンジニア認定基礎試験',
    date: '2018/10',
  },
  {
    name: 'Salesforce 認定アドミニストレーター',
    date: '2019/04',
  },
  {
    name: 'Salesforce 認定 Platform デベロッパー',
    date: '2020/12',
  },
]

/** 情報発信 */
export const OUTPUT = [
  {
    title: 'Qiita',
    link: 'https://qiita.com/yappi-dev',
  },
  {
    title: 'Zenn',
    link: 'https://zenn.dev/ko_hei',
  },
  {
    title: 'Github',
    link: 'https://github.com/yaeok',
  },
]
